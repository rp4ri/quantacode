...
- **Description**: Add `IndicatorRequest` and extend messages in `proto/marketdata.proto`
...
- **Description**: Run protoc to generate Go structs and gRPC stubs
...
- **Description**: Implement server entrypoint with gRPC server initialization on :50051
...
- **Description**: Add SIGTERM/SIGINT handling with context cancellation
- **Files**: cmd/server/main.go
...
- **Description**: Implement `internal/grpc/server/handler.go` with StreamPrices method signature
...
- **Description**: Implement connection management in `internal/infra/binance/client.go`
...
- **Description**: Add exponential backoff reconnection with max retries
- **Files**: internal/infra/binance/client.go
...
- **Description**: Implement fan-out mechanism to distribute price updates to multiple streams
- **Files**: internal/grpc/server/handler.go
...
- **Description**: Add RSI calculation in `internal/domain/indicators/rsi.go`
...
- **Description**: Add SMA calculation in `internal/domain/indicators/sma.go`
...
- **Description**: Add EMA calculation in `internal/domain/indicators/ema.go`
...
- **Description**: Implement centralized indicator calculation and subscription management
...
- **Description**: Complete `StreamPrices` method for server-side price updates
- **Files**: internal/grpc/server/handler.go
...
- **Description**: Process client requests to subscribe/unsubscribe from indicators
- **Files**: internal/grpc/server/handler.go
...
- **Description**: Send combined stream of PriceUpdate and IndicatorUpdate messages
- **Files**: internal/grpc/server/handler.go
...
- **Description**: Track active streams, notify clients, drain within timeout
...
- **Description**: Cleanly close WebSocket during shutdown
- **Files**: internal/infra/binance/client.go
...
- **Description**: Use GracefulStop() with 5s timeout
- **Files**: cmd/server/main.go
...
- **Description**: Coordinate signal handler, stream drain, Binance close, server stop
- **Files**: cmd/server/main.go
...
- **Description**: Add table-driven tests for RSI, SMA, EMA calculations
...
- **Description**: Create test that starts server, connects client, receives updates
...
- **Description**: Verify shutdown sequence with active connections
...
- **Description**: Run golangci-lint and resolve all warnings
- **Files**: All Go files
